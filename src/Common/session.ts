...
import { getEntraToken } from "./entra-auth";
...
const createDesktopDirectSession = () =>
  Session.DesktopDirect.Definition({
    authenticationProvider: new Session.EntraProvider(getEntraToken),
    takeRealTimeControl: false,
  }).getSession();

export function getSession(
  sessionType?: "platform" | "desktop" | "container" | "desktopDirect",
): Session.Session {
  ...
  switch (sType) {
	...
	case "desktopDirect":
      return createDesktopDirectSession();
    default:
      throw new Error(
        "Invalid Session Type. Should be either - platform or desktop or container or desktopDirect",
      );
  }
}
